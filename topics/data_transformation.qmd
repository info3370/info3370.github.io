...
```
```{r, echo = F}
micro <- read_dta("../data_raw/cps_00077.dta")
```
...
{{< video https://www.youtube.com/embed/DKLpzJa0UVA >}}
...
::: {.callout-note}
`filtered` should have `r prettyNum(nrow(filtered), big.mark = ",")` rows and `r ncol(filtered)` columns.
:::
...
{{< video https://www.youtube.com/embed/aLPzq7q8hik >}}
...
- Use `summarize()` to aggregate to the 10th, 50th, and 90th percentiles within each year
     - `p10 = quantile(x = incwage, probs = .1)`
     - `p50 = quantile(x = incwage, probs = .5)`
     - `p90 = quantile(x = incwage, probs = .9)`

If you are unfamiliar with `quantile()` type `?quantile` in your console for help.
...
{{< video https://www.youtube.com/embed/o4RMEKhsjjw >}}
...
{{< video https://www.youtube.com/embed/Rcor1P-q8Rc >}}
...
The `inflation_factor` tells us that \$1 in 1962 could buy about as much as \$9.67 in 2022. To take a 1962 income and report it in 2022 dollars, we should multiple it by 9.67. To do that, we first need to join our data
```{r, echo = F}
pivoted |>
  print(n = 3)
```
and merge in the inflation adjustment
```{r, echo = F}
inflation |>
  print(n = 3)
```
so that rows are correctly merged by the linking variable `year`.

Use [`left_join()`](https://r4ds.hadley.nz/joins#sec-mutating-joins) to merge `inflation_factor` onto the dataset `pivoted`. Below is a hypothetical example for the structure.
...
{{< video https://www.youtube.com/embed/gzV-F7txkBw >}}
...
{{< video https://www.youtube.com/embed/OchVyRJLi7M >}}
...
{{< video https://www.youtube.com/embed/mJlBFfoj-lc >}}
...
- explore different quantiles or other distributional summaries
- try something else, like [`geom_histogram()`](https://ggplot2.tidyverse.org/reference/geom_histogram.html) for people's incomes in a specific year

## What comes next

In the next exercise, we will use the [`asecwt`](https://cps.ipums.org/cps-action/variables/asecwt) to account for the CPS sample design and correctly
